...
class CholeskyDecomposition:
    ...
    def decompose(self):
        """Cholesky decomposition A = L L^T (real) or L L* (complex)."""
        for i in range(self.n):
            for j in range(i+1):
                s = sum(self.L[i][k] * self.L[j][k].conjugate() for k in range(j))
                if i == j:
                    val = self.A[i][i] - s
                    if abs(val) < 1e-14:
                        raise ValueError("Matrix not positive-definite!")
                    self.L[i][j] = cmath.sqrt(val)
                else:
                    self.L[i][j] = (self.A[i][j] - s) / self.L[j][j]
        return self.L
    ...
    def backward_substitution(self, y):
        """Solve L^T x = y (L^T upper-triangular)."""
        x = [0]*self.n
        for i in reversed(range(self.n)):
            s = sum(self.L[j][i].conjugate() * x[j] for j in range(i+1, self.n))
            x[i] = (y[i] - s) / self.L[i][i].conjugate()
        return x
    ...
    def solve(self, b, x0=None):
        ...
        for _ in range(self.max_iter):
            ...
            for i in range(self.n):
                ...
                x_new[i] = (b[i][0] - s)/self.A[i][i]

            # check convergence
            if max(abs(x_new[i]-x[i]) for i in range(self.n)) < self.tol:
                return x_new
            x = x_new
...
class GaussSeidelSolver:
    ...
    def solve(self, b, x0=None):
        ...
        for _ in range(self.max_iter):
            x_new = x[:]
            for i in range(self.n):
                s1 = sum(self.A[i][j] * x_new[j] for j in range(i))      # already updated
                s2 = sum(self.A[i][j] * x[j] for j in range(i+1, self.n)) # old values
                x_new[i] = (b[i][0] - s1 - s2) / self.A[i][i]
            if max(abs(x_new[i] - x[i]) for i in range(self.n)) < self.tol:
                return x_new
            x = x_new
